...
export default function NewTreatmentPlanPage() {
  ...
  const proposedCalendarEvents: ProposedEvent[] = scheduledSessions.map((sess, idx) => {
    ...
    d.setHours(h, m, 0, 0);
    const end = new Date(d.getTime() + (computedDuration > 0 ? computedDuration : 60) * 60000);
    ...
    try {
      ...
      if (scheduledSessions.length > 0) {
        ...
        if (fail > 0 && ok === 0) {
          ...
          toast(`Plan creado. ${ok} sesiones agendadas, ${fail} fallaron.`, { icon: <AlertTriangle className="w-4 h-4 text-amber-500" /> });
        } else {
          ...
          {formData.frequency && formData.startDate && formData.sessionsPlanned > 0 && (
            <div className="border-t border-gray-200 dark:border-gray-700 pt-6">
              ...
              <div className="bg-gray-50 dark:bg-gray-700/40 rounded-xl p-4 mb-4 space-y-4">
                ...
                {/* Botón generar + Ver disponibilidad */}
